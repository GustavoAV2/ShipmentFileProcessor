## calendario, com a data de expiracao que é um inteiro representando o tempo de vida da cobranca segundos a partir da data de criacao

class ProcessadorCnab:
  def __init__(self, cnab):
    self.cnab = cnab
  ...
  def processar(self):
    ...
    with open(self.root_path + file, 'r', encoding='ascii') as file:
      for line in file:
        ...
        if register_type == '0':
          self._header_process(line, dados_header)
        elif register_type == '1':
          self._detail_process(line, dados_detalhe)
